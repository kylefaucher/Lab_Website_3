...
app.get('/player_info/select_player', function(req, res) {console.log()
  var player_choice = req.query.player_choice;
  var q1 = 'select * from football_players;';
  var q2 = 'select * from football_players where id="'+player_choice+'";';
  var q3 = 'select COUNT(football_players.id) FROM football_players INNER JOIN football_games ON football_players.id::numeric = ANY(football_games.players) WHERE football_players.id = "' + player_choice +'";';
  db.task('get-everything', task => {
        return task.batch([
            task.any(q1),
            task.any(q2),
